...
class BookApi {
  ...
  static Future<Book> parseBook(url) async{
    return Book.fromJson(await DioManager.instance.get(url: "/parse/book", params: {"url": url}));
  }

  /// 获取站点列表
  static Future<List<SearchHistory>> getSites() async {
    var data = await DioManager.instance.get(url: "/parse/book/sites");
  ...
  static Future<List<SearchResult>> getSearchResults(search, site) async {
    var data = await DioManager.instance.get(url: "/parse/book/search", params: {"search": search, "site": site});
